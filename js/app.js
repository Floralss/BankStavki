// ===========================
// BLACK SOCIAL — Main App (v4 - instant UI)
// ===========================
...
function updateUI() {
  ...
  if (currentUser) {
    ...
    console.log("UI updated for:", nick, "rank:", rank.badge);
  } else {
...
async function ensureUserDoc(user, nickname) {
  ...
  try {
    ...
    if (snap.exists()) {
      ...
      if (owner && data.rankId !== "owner") {
        try { await updateDoc(ref, { rankId: "owner" }); data.rankId = "owner"; } catch(e) {}
      }
      return data;
    }
...
onAuthStateChanged(auth, async (user) => {
  ...
  if (user) {
    // Сразу ставим пользователя и обновляем UI — без ожидания базы
    currentUser = user;
    currentUserData = {
      nickname: user.displayName || (user.email ? user.email.split("@")[0] : "User"),
      rankId: isOwnerEmail(user.email) ? "owner" : "newbie",
      email: user.email,
      photoURL: user.photoURL
    };
    updateUI();

    // Потом тихо подгружаем полные данные
    ensureUserDoc(user).then(data => {
...
document.getElementById("loginForm")?.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    currentUserData = { nickname: cred.user.displayName || email.split("@")[0], rankId: isOwnerEmail(email) ? "owner" : "newbie", email };
...
document.getElementById("registerForm")?.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    currentUser = cred.user;
    currentUserData = { nickname, rankId: isOwnerEmail(email) ? "owner" : "newbie", email };
    authModal?.classList.remove("active");
    updateUI();
    ...
    let msg = err.message;
    if (err.code === "auth/email-already-in-use") msg = "Email уже зарегистрирован — используй «Вход»";
    toast(msg, "error");
  }
});
...
async function googleAuth() {
  try {
    ...
    currentUserData = {
      ...
      rankId: isOwnerEmail(result.user.email) ? "owner" : "newbie",
...
async function loadRecentTopics() {
  ...
  try {
    ...
    snap.forEach(docSnap => {
      ...
      el.innerHTML = '<div class="topic-avatar">' + getInitials(t.authorNickname) + '</div><div class="topic-body"><div class="topic-title">' + escapeHtml(t.title) + '</div><div class="topic-meta"><span class="author">' + escapeHtml(t.authorNickname || "") + '</span> · ' + formatDate(t.createdAt) + '</div></div><div class="topic-stats">' + (t.repliesCount || 0) + ' отв.</div>';
      el.onclick = () => openTopic(docSnap.id);
      container.appendChild(el);
    });
  } catch (err) {
    console.warn("topics:", err.message);
  }
}
...
document.getElementById("createTopicForm")?.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    await addDoc(collection(db, "posts"), {
      ...
      authorNickname: currentUserData?.nickname || "User",
      authorRankId: currentUserData?.rankId || "newbie",
      createdAt: serverTimestamp(), isFirst: true
    });
...
async function openTopic(topicId) {
  ...
  try {
    ...
    contentEl.innerHTML = '<div class="topic-header"><h2>' + escapeHtml(topic.title) + '</h2><div class="meta">Автор: <span style="color:var(--accent)">' + escapeHtml(topic.authorNickname) + '</span></div></div>' + postsHtml +
      (currentUser ? '<div class="reply-form"><form id="replyForm"><div class="form-group"><textarea id="replyContent" rows="4" required placeholder="Ваш ответ..."></textarea></div><button type="submit" class="btn btn-primary">Отправить</button></form></div>' : '<p style="color:var(--text-muted)">Войдите, чтобы ответить</p>');
...
console.log("BLACK SOCIAL v4 loaded");
